...
RAIL is a monorepo for the headless knowledge runtime: FastAPI backend, Python engine, `rail` CLI, and optional MCP server.
...
**Requirements:** Python 3.11+, git. Convex URL for cloud mode (or local project folders).
...
The API starts on **http://localhost:8000**. Use `rail --help` for CLI access.
...
The project pipeline endpoint and CLI hydration commands:
...
| `rail` | `pip install -e packages/rail-py` | Included in `install-rail.sh` |
...
- Install Python manually, then `make install` from PowerShell.
