...
| Valid/known time, deletion/revocation/invalidation | explicit snapshot admission; authorized `objects_in_region` uses candidate refs then existing reader/invalidation checks; expired and frame-mismatched cases conservatively fall back | Partial: no direct projection subscription or tombstone index. |
