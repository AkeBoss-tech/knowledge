...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="krail-spatial-benchmark-") as directory:
        ...
        scene_diff = lambda: memory.scene_diff(world_id="bench", session_id="bench-session", before_scene_ref=memory._scenes[0].scene_ref, after_scene_ref=memory._scenes[-1].scene_ref, before_at=NOW, after_at=NOW, known_at=NOW, reader=Allow())
        ...
        scene_diff_timing = elapsed_ms(scene_diff, args.repeats)
        ...
        payload = {
            ...
            "fixture": {"initial_objects": args.objects, "observations_per_initial_object": args.observations, "appearance_records": len(memory._appearance_records), "scene_snapshots": len(memory._scenes), "initial_external_asset_refs": len(assets), "current_world_records_after_mutations": len(memory._records), "embedded_asset_bytes": 0},
            "public_authorized_region_query": public_timing,
            "public_authorized_appearance_gallery": appearance_timing,
            "public_authorized_finite_fixture_history": history_timing,
            "public_authorized_scene_read": scene_timing,
            "public_authorized_scene_diff": scene_diff_timing,
