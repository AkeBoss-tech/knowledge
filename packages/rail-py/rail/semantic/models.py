...
class Entity(ScopedModel):
    ...
    merged_into: NonEmpty | None = None
    provenance: EvidenceProvenance
    revision: int = Field(ge=1)
...
class OntologyMigrationProposal(StrictModel):
    ...
    to_version: NonEmpty
    operations: tuple[dict[str, Any], ...] = Field(min_length=1, max_length=256)
    reversible: bool
    ...
    def reversible_has_rollback(self) -> OntologyMigrationProposal:
        if self.reversible and not self.rollback_operations:
            raise ValueError("reversible migration proposals require rollback operations")
...
class OntologyChangeOperation(StrictModel):
    ...
    def operation_shape_is_valid(self) -> OntologyChangeOperation:
        ...
        if self.operation in {"update", "deprecate", "restore"} and self.before_digest is None:
...
class OntologyChangeSet(ScopedModel):
    ...
    operations: tuple[OntologyChangeOperation, ...] = Field(min_length=1, max_length=512)
...
class OntologyPackage(ScopedModel):
    ...
    def lifecycle_is_consistent(self) -> OntologyPackage:
        ...
        if reviewed and (self.reviewer is None or self.review_digest is None):
            raise ValueError("reviewed packages require reviewer and review digest")
        if self.state == "published" and self.published_version != self.proposed_version:
            raise ValueError("published package must bind the reviewed proposed version")
